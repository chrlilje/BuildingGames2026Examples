...
1. `index.html`
   - Load p5.js first.
   - Load `scenes.js` before `main.js`.

2. `main.js`
   - `setup()` creates canvas and starts with `switchScene(scene1)`.
   - `draw()` only calls `currentScene.render()` if it exists.
...
3. `scenes.js`
   ...
   - Define explicit scene objects (`scene1`, `scene2`, `scene3`).
   - Each scene should have `enter`, `render`, `keyPressed`, and `exit` methods.

### Optional file split
- It is fine to split scenes into individual files (for example: `scene1.js`, `scene2.js`, `scene3.js`).
- If scenes are split, keep the same scene method style and scene-switch pattern.
- Ensure script load order still works (scene files loaded before `main.js`).
- Keep this optional; the default beginner setup should still work with a single `scenes.js` file.

Example script order in `index.html` when split:
...
- Put scene-specific key behavior inside each scene’s `keyPressed` method.
...
- Keep file count small; only add files when they improve clarity.
- No extra features, UI systems, or abstractions not asked for.

## Default 3-scene behavior
For a basic 3-scene starter:
- Start in `scene1`.
- In each scene’s `keyPressed`:
  - key `'1'` switches to `scene1`
  - key `'2'` switches to `scene2`
  - key `'3'` switches to `scene3`
- In each `render`, clearly draw scene name text (e.g., "Scene 1").
